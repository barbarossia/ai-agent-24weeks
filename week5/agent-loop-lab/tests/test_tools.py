...
from agent_loop_lab.tools import (
    TOOLS,
    ToolExecutionError,
    check_alerting,
    get_dns_result,
...
def test_check_alerting_known_host():
    result = check_alerting("db01")
    assert result["alerts"] == [{"level": "warning", "message": "disk usage 82%"}]


def test_check_alerting_buggy01_raises_unexpected_exception():
    """buggy01 triggers a plain RuntimeError, not ToolExecutionError — this
    is the "unexpected tool bug" scenario the agent loop must also survive.
    """
    with pytest.raises(RuntimeError):
        check_alerting("buggy01")
